...
# Current production model can be overridden in Streamlit Secrets/.env.
# Example:
...
# HyDE adds an additional Groq request before retrieval.
# Disabled by default because direct vector retrieval + reranking
# is faster and avoids an unnecessary point of failure.
...
def get_vector_store(namespace: str):
    """
    Cache PineconeVectorStore objects.

    This prevents recreating the vector-store wrapper on every
    Streamlit rerun/question.
    """
...
def get_llm(api_key: str):
    """Create one cached Groq client/model per API key."""
...
def generate_document_id(*parts: str) -> str:
    """
    Stable ID using source + page + chunk content.

    Including source/page avoids collisions when two PDFs contain
    identical text.
    """
...
def ingest_pdfs(uploaded_files, namespace: str = PDF_NAMESPACE) -> int:
    """
    Load uploaded PDFs, split them, and add chunks to Pinecone.

    Uses temporary files instead of writing uploaded files into the
...
def direct_retrieve(question: str, k: int = RETRIEVE_K):
    """
    Fast path: embed the user's actual question and search Pinecone.

    This replaces the old mandatory HyDE LLM call.
    """
...
def hyde_retrieve(question: str, k: int = RETRIEVE_K):
    ...
    Only enabled with USE_HYDE=true. If HyDE fails, automatically
    falls back to direct retrieval.
    """
    if not USE_HYDE:
    ...
    try:
        ...
        # Retrieval should remain usable even if the optional HyDE
        # request fails.
        return direct_retrieve(question, k)
...
def rerank_documents(
    ...
    # Avoid unnecessary model work when the vector search returned
    # fewer documents than the requested final result.
...
def generate_final_answer(
    ...
    if not context_docs:
        return (
            "I could not find relevant information in the EV "
...
with st.sidebar:
    ...
    is_admin = (
        st.session_state.upload_authorized
        or st.session_state.feedback_authorized
    )
    ...
    if is_admin:
        ...
        if admin_key:
            ...
            st.error(
                "GROQ_API_KEY is not configured in Streamlit Secrets/.env."
            )
        ...
        if user_key:
            ...
            if cleaned:
                ...
                st.error("❌ Invalid/empty Groq key.")
    ...
    if st.button("🔓 Verify Upload Password"):
        ...
        # Do NOT use a hard-coded production password.
    ...
    if st.session_state.upload_authorized:
        ...
        if uploaded_files and st.button("🚀 Add to Vector DB"):
            with st.spinner(
                f"Processing {len(uploaded_files)} PDF(s)..."
            ):
                try:
                    ensure_index()
                    ...
                    if count:
                        st.success(
                            f"✅ Added {count} chunks to Pinecone."
                        )
                    else:
                        st.warning(
                            "No readable text chunks were found."
                        )
...
if not st.session_state.groq_api_key:
    st.warning(
        "⚠️ Enter a valid Groq API key in the sidebar to start chatting."
    )
    st.stop()
    ...
    with st.chat_message("assistant"):
        ...
        if st.session_state.feedback_enabled:
            ...
            with col1:
                if st.button(
                    "👍 Good",
                    key=f"good_{hashlib.md5(prompt.encode()).hexdigest()}",
                ):
                    try:
            ...
            with col2:
                if st.button(
                    "👎 Wrong",
                    key=f"wrong_{hashlib.md5(prompt.encode()).hexdigest()}",
                ):
        ...
        if st.button("Submit Correction"):
            if correction.strip():
                try:
                    ...
                    st.success(
                        "✅ Correction saved for future similar questions."
                    )
